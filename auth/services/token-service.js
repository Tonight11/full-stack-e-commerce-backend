...
class TokenService {
    getTokens(payload) {
        const accesToken = jwt.sign(payload, process.env.SECRET_ACCES, {expiresIn: '1h'})
        const refreshToken = jwt.sign(payload, process.env.SECRET_REFRESH, {expiresIn: '15d'})
    ...
    checkRefresh(refresh) {
        try {
            const refreshToken = jwt.verify(refresh, process.env.SECRET_REFRESH)
            return refreshToken
        } catch (e) {
            return null
        }
